...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--simulate") {
            ...
            trainPolicies = true;
            if (i + 1 < argc && looksLikeNumber(argv[i + 1])) {
                trainEpisodes = std::max(1, std::stoi(argv[++i]));
            }
            if (i + 1 < argc && looksLikeNumber(argv[i + 1])) {
                trainSeconds = std::max(60.f, std::stof(argv[++i]));
            }
            if (i + 1 < argc && looksLikeNumber(argv[i + 1])) {
                trainSeed = static_cast<unsigned int>(std::stoul(argv[++i]));
            }
        }
    }
    ...
    if (trainPolicies) {
        runPolicyTraining(trainEpisodes, trainSeconds, simulateDt, trainSeed);
        return 0;
    }
